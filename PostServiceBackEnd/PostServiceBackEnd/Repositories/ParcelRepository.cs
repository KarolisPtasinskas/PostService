...
namespace PostServiceBackEnd.Repositories
{
    public class ParcelRepository : IParcelRepository<Parcel>
    {
        ...
        {
            return await _dataContext.Parcels.OrderByDescending(p => p.Weight).Include(pm => pm.ParcelMachine).ToListAsync();
        }

        public async Task<List<Parcel>> GetAllFilteredAsync(ParcelFilter parcelFilter)
        {
            return await _dataContext.Parcels.Where(p => p.ParcelMachineId == parcelFilter.ParcelMachineId).OrderByDescending(p => p.Weight).Include(pm => pm.ParcelMachine).ToListAsync();
        }
